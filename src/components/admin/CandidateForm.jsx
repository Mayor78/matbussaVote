    ...
    if (!propElectionId) {
      const fetchElections = async () => {
        try {
          ...
          if (import.meta.env.DEV) console.error('Error fetching elections:', error);
        } finally {
  ...
  useEffect(() => {
    ...
    const fetchPositions = async () => {
      try {
        ...
        const data = await positionService.getPositionsByElection(selectedElectionId);
        setPositions(data);
      } catch (error) {
        if (import.meta.env.DEV) console.error('Error fetching positions:', error);
      } finally {
        setLoadingPositions(false);
      }
    };
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      {!propElectionId && (
        <div>
          ...
          {loadingElections ? (
            <div className="text-sm text-gray-500 py-2">Loading elections...</div>
          ) : (
            <select
              value={selectedElectionId}
              onChange={(e) => {
                setSelectedElectionId(e.target.value);
                reset((prev) => ({ ...prev, positionId: '' }));
              }}
            ...
            >
              ...
              {elections.map(el => (
                <option key={el.id} value={el.id}>{el.title} ({el.academic_session})</option>
              ))}
            </select>
          )}
        </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          {loadingPositions ? (
            <div className="text-sm text-gray-500 py-2">Loading positions...</div>
          ) : (
            <select
              {...register('positionId')}
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500"
      ...
      <div>
        ...
        <textarea
          {...register('manifesto')}
          placeholder="Enter candidate manifesto, key goals, or short biography..."
          className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
          rows="5"
        />
